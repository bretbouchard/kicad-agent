...
# ---------------------------------------------------------------------------
# Answer templates per QA type (Council HIGH-3 requirement)
# ---------------------------------------------------------------------------
...
_VIOLATION_ROOT_CAUSES: dict[str, dict[str, str]] = {
...
_DIFFICULTY_RULES: dict[str, dict[str, str]] = {
    ...
    def _generate_signal_flow_qa(
        ...
        for sc in subcircuits:
            ...
            for q_template in _QUESTION_TEMPLATES["signal_flow"]:
                ...
                answer = a_template.format(
                    input_net=input_net,
                    path_components=path_str.split(" -> ")[1:-1] if " -> " in path_str else path_str,
                    output_net=output_net,
                    subcircuit_name=sc_name.replace("_", " "),
                ...
                # Fix: path_components is a list, join it for the answer
                if isinstance(answer, str) and "path_components" in a_template:
                    # The template uses {path_components} as a slot -- reformat
                    path_comp_str = " -> ".join(
                        f"{c['ref']}" for c in components
                    )
                    answer = answer.replace(
                        str(path_str.split(" -> ")[1:-1]),
                        path_comp_str,
                    )
                # Rebuild answer cleanly
