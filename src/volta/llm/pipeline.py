...
def llm_generate(
    ...
    try:
        if intent_parser is None:
            if hybrid_client is not None:
                from volta.llm.unified_parsers import UnifiedIntentParser
                intent_parser = UnifiedIntentParser(hybrid_client)
            else:
                from volta.llm.intent_parser import IntentParser
                from volta.llm.provider import get_provider
    ...
    if run_refinement and not erc_passed:
        ...
        if sch_path is not None and sch_path.exists():
            ...
            if fixer is None:
                if hybrid_client is not None:
                    from volta.llm.unified_parsers import UnifiedErrorFixer
    ...
    if run_critique and generation_result.pcb_path is not None and generation_result.pcb_path.exists():
        try:
            if design_critic is None:
                if hybrid_client is not None:
                    from volta.llm.design_critic import DesignCritic
    ...
    if run_evaluation:
        try:
            from volta.generation.evaluation import evaluate_design
...
def _resolve_hybrid_client(
    ...
    try:
        ...
        from volta.llm.backend import HybridLLMClient
...
def _attempt_export(generation_result: Any, errors: list[str]) -> None:
    ...
    if generation_result.pcb_path is not None:
        try:
            from volta.export import export_gerber
    ...
    if generation_result.schematic_path is not None:
        try:
            from volta.export import export_bom
