...
def _handle_collect(argv: list[str]) -> None:
    ...
    from volta.training.real_dataset import run_pipeline
...
def _handle_component_search(argv: list[str]) -> None:
    ...
    from volta.mcp.server import main as mcp_main
    mcp_main()
...
def _handle_ai_stats(argv: list[str]) -> None:
    ...
    parser.add_argument(
        "--dir",
        type=Path,
        default=Path(".volta_tracking"),
        help="Tracking data directory (default: .volta_tracking)",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    from volta.ai_tracking.tracker import InterventionTracker
    from volta.ai_tracking.gap_analyzer import GapAnalyzer
...
def _handle_check_conventions(argv: list[str]) -> None:
    ...
    from volta.cli.check_conventions_cmd import (
        register_parser,
...
def _handle_gate(argv: list[str]) -> None:
    ...
    if args.action == "run":
        ...
        from volta.validation.gate_runner import get_gate_runner
        from volta.ops.handlers.gate_handlers import _detect_design_stage, _suggest_next_actions
...
def _handle_demo(argv: list[str]) -> None:
    ...
    from volta.demo.templates import list_templates as _list_templates
