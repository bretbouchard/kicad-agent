...
2. Sidecar file .volta/constraints.json for reliable round-trip
...
from volta.validation.gates.constraint_schema import DesignConstraints

logger = logging.getLogger(__name__)

# Sidecar file path (relative to project_dir)
_CONSTRAINTS_SIDECAR = ".volta/constraints.json"
...
class SetConstraintsOp(BaseModel):
    ...
    .volta/constraints.json for reliable round-trip (88-02-M2).
...
class GetConstraintsOp(BaseModel):
    ...
    Reads from the persisted sidecar file .volta/constraints.json.
...
def handle_set_constraints(op: SetConstraintsOp) -> dict[str, Any]:
    ...
    5. Write canonical JSON to .volta/constraints.json sidecar file.
    ...
    from volta.validation.gates.constraint_gate import ConstraintPropagator
...
def handle_get_constraints(op: GetConstraintsOp) -> dict[str, Any]:
    ...
    Reads from .volta/constraints.json and returns the deserialized
