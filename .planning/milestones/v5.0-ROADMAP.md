...
- [x] **Phase 17: Package & Distribution** - PyPI publish, CLI entry point, pip install volta
...
- [x] **Phase 22: Agent Integration + End-to-End Evaluation** - Wire fine-tuned model into volta as reasoning engine
...
**Milestone Goal:** Fill the five operation gaps so volta handles real-world KiCad projects with hierarchical designs and full CRUD capabilities. Zero new dependencies. **1673 tests, 57 operation types, 14 schema sub-modules.**
...
**Milestone Goal:** Expose all 57 volta operations as MCP tools so any AI agent (Claude, Cursor, etc.) can invoke KiCad file edits directly. Zero new dependencies. ~250 lines new code.
...
**Goal**: The volta is invokable from any KiCad project via the GSD Skill interface and from the terminal via CLI
**Depends on**: Phase 6
**Requirements**: SKILL-01, SKILL-02, SKILL-03, SKILL-04
**Success Criteria** (what must be TRUE):
  1. A GSD Skill manifest declares all volta capabilities
...
**Goal**: Make volta installable via pip with a proper CLI entry point and PyPI package
**Depends on**: Phase 7
**Requirements**: DIST-01, DIST-02, DIST-03, DIST-04
**Success Criteria** (what must be TRUE):
  1. `pip install volta` installs a working package with CLI entry point
  2. `volta` CLI command runs operations, validation, and project context
...
**Goal**: Wire the GRPO-trained LLM into volta as its reasoning engine with best-of-N generation
...
**Success Criteria** (what must be TRUE):
  ...
  3. volta CLI has `analyze` subcommand using the fine-tuned model
  4. Python API exposes `generate_analysis(pcb_path)` returning scored chains
  5. GSD Skill: Claude can invoke `/volta analyze <pcb>` and get spatial reasoning
  6. End-to-end demo: analyze HackRF One and produce quality reasoning chain
**Plans**: 2 plans (2/2 complete)

Plans:
- [x] 22-01-PLAN.md -- Inference wrapper + best-of-N + volta wiring
...
**Goal**: New MCP server binary exposing all 57 volta operations as individually named tools, with dynamic schema generation, structured error handling, and meta-tools for schema discovery and project context
...
**Success Criteria** (what must be TRUE):
  ...
  2. MCP server runs on stdio transport via `volta-edit` CLI entry point
  ...
  11. `volta-edit` entry point registered in pyproject.toml with no new dependencies
...
**Success Criteria** (what must be TRUE):
  ...
  5. CLI: `python -m volta.benchmarks --dataset <file> --model <name>`
**Plans**: 2 plans (2/2 complete)
...
**Success Criteria** (what must be TRUE):
  ...
  6. CLI subcommand `volta dfm <board>` works end-to-end
...
**Success Criteria** (what must be TRUE):
  ...
  2. volta --help lists all subcommands with descriptions
  3. volta component-search --help shows help, does NOT start server
**Plans**: 3 plans
...
**Goal**: Harden PersistentUndoStack with comprehensive tests and add `volta undo`/`volta redo` CLI commands
...
**Success Criteria** (what must be TRUE):
  ...
  5. `volta undo` CLI command works end-to-end
  6. `volta redo` CLI command works end-to-end
  7. `.volta/` auto-added to `.gitignore`
...
**Source**: `hardware/dumb-cartridges/aether-drive/BUGS.md` section "volta Source Review (2026-06-06)"
...
**Success Criteria** (what must be TRUE):
  ...
  6. Works on PCBs from Freerouting, volta A*, or custom auto_route.py
  7. CLI: `volta analyze-gaps <pcb_file> [--render-gaps]`, MCP tool: `analyze_gaps`
**Plans**: 2 plans
...
**Success Criteria** (what must be TRUE):
  1. GapFiller maps gap types to existing volta operations
  ...
  11. CLI: `volta fill-gaps <pcb_file> [--dry-run] [--model gemma4|qwen]`
**Plans**: 3 plans
...
**Goal:** Wire gap-filling pipeline into analog-ecosystem's PCB workflow, replacing per-board custom scripts with unified volta workflow. Model config (Gemma 4 12B vision + Qwen2.5-0.5B text) in volta.yaml.
**Depends on:** Phase 82 (gap-filling pipeline)
**Requirements:** WORKFLOW-01, WORKFLOW-02, WORKFLOW-03
**Success Criteria** (what must be TRUE):
  1. Single command: `volta workflow route-and-fill <pcb>`
  ...
  4. Board config from .kicad_pro or volta.yaml (routing + model selection)
...
**Success Criteria** (what must be TRUE):
  ...
  6. Adapter stored on `/Volumes/Storage/models/volta/adapters/` (not local SSD)
**Plans**: 2 plans
...
**Success Criteria** (what must be TRUE):
  ...
  2. `volta status` shows current stage and blockers
...
**Milestone Goal:** Adopt the proven Vast.ai GPU training flow (from spectral-primitives) for volta's LoRA training, and move all model artifacts to external storage.
...
- [x] **Phase 97: External Model Storage** - Move all trained adapters and training datasets to `/Volumes/Storage/models/volta/`. Adapter metadata registry with versioning. (completed 2026-06-19)
...
**Success Criteria** (what must be TRUE):
  1. Adapters at `/Volumes/Storage/models/volta/adapters/` with metadata registry
  2. Training datasets at `/Volumes/Storage/models/volta/datasets/`
...
### Phase 108: Deterministic Autolayout Engine — IN PROGRESS (Task 2 in review)

**Goal:** Build Sugiyama-framework-based autolayout engine on existing `schematic_routing/` primitives (~6,000 LOC from Phase 38: SchematicGraph, wire_router, power_unit_placer, net_namer, net_resolver, netlist_parser). Output passes Phase 48.5 SchematicReadabilityScorer within 0.10 SRS of human-expert baseline on Phase 93 golden boards (D-03). New `auto_layout_sch` op registers in `src/volta/ops/registry.py`.
**Requirements**: TBD (deferred to /gsd-discuss-phase 108)
**Depends on:** Phase 107
**Plans:** 4/4 plans created
**Status:** Wave 1-4 implemented. Plan 04 Task 1 (verification script + geometry gate) COMPLETE. Task 2 (human visual verify) IN REVIEW — components on-page + non-stacked (hard gate passes), but subcircuit-group placement overlapping + loose-component parking needs layout refinement before human approval. All deferred items (D-1, D-2, D-4..D-6) IMPLEMENTED. 319/319 tests pass.

Plans:
- [x] 108-01-PLAN.md — Sugiyama 5-stage algorithm core (networkx, no Graphviz) — Wave 1 — COMPLETE
- [x] 108-02-PLAN.md — Three low-level ops: place_components_sch, route_wires_sch, apply_labels_sch — Wave 2 — COMPLETE
- [x] 108-03-PLAN.md — auto_layout_sch orchestrator + HierarchicalSheetSplitter (D-02) — Wave 3 — COMPLETE
- [x] 108-04-PLAN.md — D-03 SRS verification within 0.10 of human-expert baseline — Wave 4 — Task 1 COMPLETE, Task 2 IN REVIEW

Branch: `phase-108-task2-on-page-guarantee` (2 commits ahead of master: `2a752dfd` on-page guarantee, `4ec30bd5` symbol normalizer).

Next step: fix subcircuit-group placement (x_offset not separating groups) + park loose components below the placed topology area instead of stacking them at the same Y. Then re-render PDFs for human approval.
...
- [ ] 111-03-PLAN.md — Wave 3: `volta check-conventions` CLI + Phase 108 autolayout integration (conventions as placement constraints)
...
- [ ] **P1-R2-1: IEEE315_PIN_ORIENTATION_01 TRANSFORM semantics** — Trigger: Phase 115 lands. Signal: `SchematicRawWriter._move_symbol_by_ref` honors `angle` argument (currently preserves existing rotation via regex group(4) at line 754-757, ignores caller-supplied angle). Readiness: `dataclasses.replace`-snapped orientation in `apply()` becomes round-trippable to disk. v1 demoted to read-only (apply = identity) per Council Gate 1 Round 2 finding — writer cannot round-trip orientation changes. Source: `src/volta/conventions/catalog/pin_orientation.py:21`. Deferred: 2026-07-04.
