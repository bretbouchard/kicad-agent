       ...
       class CorpusCurator:
           ...
           def download_and_validate(
               ...
               if not any(domain == allowed or domain.endswith(f".{allowed}") for allowed in ALLOWED_DOMAINS):
                   logger.warning(f"Rejected {name}: domain '{domain}' not in allowed list {ALLOWED_DOMAINS}")
                   return None
               ...
               with tempfile.TemporaryDirectory(prefix=f"corpus_{name}_") as tmpdir:
                   ...
                   # File size verification: reject repos exceeding size limit
                   total_size = sum(f.stat().st_size for f in local.rglob("*") if f.is_file())
                   ...
                   # SHA256 integrity hash of all downloaded files for dedup verification
...
| T-53-02 | Tampering | downloaded KiCad files | mitigate | SHA256 content hash computed on all downloaded files; hash stored in CuratedProject metadata for integrity verification |
...
| T-53-05 | DoS | large repos | mitigate | MAX_DOWNLOAD_SIZE=50MB enforced after download; repos exceeding limit are rejected |
| T-53-06 | Spoofing | malicious download URLs | mitigate | URL domain validation: only github.com and hackaday.io domains allowed; others rejected |
| T-53-07 | Tampering | supply chain attack via repo substitution | mitigate | SHA256 hash verifies downloaded content matches expected repo |
</threat_model>
