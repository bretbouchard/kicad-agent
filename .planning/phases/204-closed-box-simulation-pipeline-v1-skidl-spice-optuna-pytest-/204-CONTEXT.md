...
Build the **optimization + testing + dataframe + demo layer** ON TOP of Phase 158's existing `src/volta/spice/` foundation. Prove the closed-box magic end-to-end on a **Eurorack input preamp** canonical example: "give me a 20 dB preamp" → Optuna sweeps E12 resistor values → ngspice verifies → pytest asserts → Bode PNG + BOM markdown emit.

**Phase 204 does NOT:**
- Rewrite or replace `src/volta/spice/` (Phase 158 foundation is consumed as-is)
...
- `src/volta/spice/__init__.py` — Public API (15 symbols: run_simulation, generate_*_testbench, compute_degradation, is_simulatable, get_model, UNSIMULATABLE, types)
- `src/volta/spice/types.py` — Canonical result types (`SimulationResult`, `AnalysisResult`, `Trace`, `DegradationReport` — frozen dataclasses)
- `src/volta/spice/ngspice_runner.py` — ngspice CLI subprocess wrapper (THE entry point for sims)
- `src/volta/spice/testbench.py` — AC/TRAN/NOISE/THD testbench generators (USE `generate_ac_testbench`)
- `src/volta/spice/model_registry.py` — SPICE model lookup (add `2N3904` here or rely on ngspice built-in)
- `tests/spice/test_spice.py` — BLK-1 strict test pattern (model for new tests)

### Existing SKiDL Integration
- `src/volta/circuit_ir/skidl_circuit.py` — Build skidl.Circuit from KiCad schematic (lines 39-89)
- `src/volta/circuit_ir/skidl_emitter.py` — Emit SKiDL Python source (lines 69, 150 reference KiCad symbol paths)
- `src/volta/circuit_ir/topology_from_skidl.py` — skidl.Circuit → CircuitTopology (lines 38, 196)
...
import pytest
from volta.sim.eurorack import build_preamp_circuit, build_testbench
from volta.spice import run_simulation, SimulationResult
